...
void processRequest(int pipefd, SSL* ssl, int sockfd, int tunfd)
{
  /* char buf[1024];
   ...
   while(1) {
     ...
     if (FD_ISSET(pipefd,  &readFDSet)) pipeSelected(pipefd, sockfd, ssl);
     if (FD_ISSET(sockfd, &readFDSet)) socketSelected(pipefd, sockfd, ssl, tunfd);
   }
...
//     write(fd[1], string, (strlen(string)+1)); ➁
	 ...
	 //  close(fd[0]); // Close the input end of the pipe.
	   ...
	   struct sockaddr_in sa_client;
		   ...
		   processRequest(fd[0], ssl, newsock, tunfd);
		   closeSSLAndSocket(ssl, newsock);
		   printf("Cleared socket and ssl structures.. End of child process\n");
		   return 0;
		   /*   FD_ZERO(&readFDSet);
